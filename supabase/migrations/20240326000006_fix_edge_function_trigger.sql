-- Revert back to notification-based trigger without direct HTTP call
...
BEGIN
    ...
    IF user_role = 'customer' THEN
        ...
        -- Notify Edge Function through notification channel
        PERFORM pg_notify(
            'new_customer_message',  -- Channel name that Edge Function listens to
            json_build_object(
                'message_id', NEW.id,
                'ticket_id', NEW.ticket_id,
                'suggestion_id', v_suggestion_id,  -- Include for direct update
        ...
        RAISE NOTICE 'Created AI suggestion % and notified Edge Function for message %', v_suggestion_id, NEW.id;
    ELSE
